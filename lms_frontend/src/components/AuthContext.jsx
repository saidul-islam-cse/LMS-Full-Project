...
export function AuthProvider({ children }) {
  ...
  const addCourse = async (title, description, banner, price, duration, categoryId) => {
    // Send POST request to register a new user
    await axios.post("http://127.0.0.1:8000/api/courses/", { title, description, banner, price, duration, categoryId });
  ...
  return (
    <AuthContext.Provider value={{ token, login, logout, register, addCourse, page, goTo }}>
      {children}
